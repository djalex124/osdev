...
void kscreen_init()
{
    ...
    uint64_t fb_aligned = ((kgraphics.horizontal_res * kgraphics.vertical_res * 4) + 0x1000 - 1) / 0x1000;
    ...
    kscreen_buffer = kmem_alloc(fb_aligned);
    kscreen_termbuffer = kmem_alloc(fb_aligned);

    k_infotable.k_graphics = &kgraphics;
