...
void kacpi_startaps(uint8_t total_processors)
{
    ...
    for (int i = 0; i < total_processors; i++)
    {
        ...
        lapic_write(lapic_base, 0x280, 0);
        lapic_write(lapic_base, 0x310, (lapic_read(lapic_base, 0x310) & 0xFFFFFF) | (i << 24));
        lapic_write(lapic_base, 0x300, (lapic_read(lapic_base, 0x300) & 0xFFF00000) | 0xC500);
        do {asm volatile ("pause" : : : "memory");}
        while (lapic_read(lapic_base, 0x300) & (1 << 12));
        lapic_write(lapic_base, 0x310, (lapic_read(lapic_base, 0x310) & 0xFFFFFF) | (i << 24));
        lapic_write(lapic_base, 0x300, (lapic_read(lapic_base, 0x300) & 0xFFF00000) | 0x8500);
        do {asm volatile ("pause" : : : "memory");}
        while (lapic_read(lapic_base, 0x300) & (1 << 12));
        ksleep(10);
        for (int j = 0; j < 2; j++)
        {
            lapic_write(lapic_base, 0x280, 0);
            lapic_write(lapic_base, 0x310, (lapic_read(lapic_base, 0x310) & 0xFFFFFF) | (i << 24));
            lapic_write(lapic_base, 0x300, (lapic_read(lapic_base, 0x300) & 0xFFF0F800) | 0x608);
            ksleep(1);
            do {asm volatile ("pause" : : : "memory");}
            while (lapic_read(lapic_base, 0x300) & (1 << 12));
        }
...
void kacpi_apic_eoi()
{
    lapic_write(kacpi_lapic_base, 0xB0, 0);
}
...
void kacpi_processapic(acpi_madt *madt)
{
    ...
    lapic_write(kacpi_lapic_base, 0xF0, lapic_read(kacpi_lapic_base, 0xF0) | 0x1FF);
    lapic_write(kacpi_lapic_base, 0xB0, 0);
